...
数カ月前に、運営様開催の「技術書を執筆しよう！初心者向け勉強会」@<fn>{connpass}に参加させていただきました。
それをきっかけに、本公式ガイドブックに寄稿する機会をいただけました。運営様ありがとうございました。  


さて、私は趣味でプログラミング言語Haskellで電子文書リーダーを開発しています（@<href>{https://www.polymony.net）。, https://www.polymony.net）。}
...
そういう速読術を極めていない人@<fn>{1}でも、これらの本を読みやすくする方法を発見したのでご紹介します。
...
技書博は @<strong>{技術書を頒布される方のみサークル出展いただけます} 。  


出展者は @<strong>{各自の良識の範囲で「技術」を定義し} 、自らがもつ技術を共有する場としてご利用ください。  


私たち運営事務局は、 @<strong>{エンジニアによるアウトプットを推進し} 、 @<strong>{知識の共有によってより良いエンジニアコミュニティを築き} 、また参加される皆さんが @<strong>{共に成長することを望んでいます} 。  
...
技書博は @<strong>{「技術書を頒布される方」} のみサークル出展いただけます。  


出展者は @<strong>{各自の良識の範囲} で @<strong>{「技術」} を定義し、自らがもつ技術を共有する場としてご利用ください。  


私たち運営事務局は、 @<strong>{エンジニア} による @<strong>{アウトプット} を推進し、 @<strong>{知識の共有} によって @<strong>{より良いエンジニアコミュニティ} を築き、また参加される皆さんが @<strong>{共に成長すること} を望んでいます。  
...
We make @<strong>{some toy programs} in @<strong>{Haskell}. @<strong>{The current one} is @<strong>{a viewer} for @<strong>{PDF Documents}, characterized by @<strong>{ad hoc coloring} of @<strong>{noun phrases} for @<strong>{fast and accurate reading}. There is @<strong>{another delimiter} other than @<strong>{space}.  
...
次に、名詞句単位という縛りに加えて、読者側が着色モード@<fn>{2}を把握していることも重要です。まずは着色モードの一例として下の文章(ケースE)を参照ください。  
...
サークル参加は @<strong>{有料} 。「通常スペース」 @<strong>{出展料：5000円} 、.. 「ゆったりスペース」 @<strong>{出展料：8000円} の２種類あり、..申込期限は@<u>{9月22日} 
...
これは、完全一致で「円」、「料」、は@<strong>{ボールド}、数字は @<i>{イタリック} 、「月」「日」は @<u>{アンダーバー} になるよう指定した場合です。  
...
@<u>{速読}、@<u>{精読}の対象として、@<strong>{専門書}や@<strong>{技術書}を想定しています。一部の @<u>{速読エリート} を除けば、これらの本を@<u>{速読}するのは難しいと見ています。
...
@<i>{速読}、@<strong>{精読}の@<u>{対象}として、@<i>{専門書}や@<strong>{技術書}を@<u>{想定}しています。@<i>{一部}の@<strong>{速読エリート}を除けば、これらの@<u>{本}を@<i>{速読}するのは難しいと見ています。


@<strong>{そういう速読術}を極めていない@<u>{人}でも、これらの@<i>{本}を@<strong>{読み}やすくする@<u>{方法}を@<i>{発見}したので@<strong>{ご紹介}します。
 ...
 * @<strong>{筆者(プログラマ)}が@<strong>{執筆時（コンパイル時）}に@<strong>{スタティックに用意するもの}ではなく、
 * @<strong>{読者（ユーザー）}が@<strong>{読書時（ランタイム時）}に@<strong>{ダイナミックに主体的に行うもの}である、ということです@<fn>{3}。
...
私は以前は完全なペーパー主義者でした。が、上記技術を使った電子書籍リーダを開発し始めてからは、電子版（pdf版)も購入するようになりました@<fn>{4}。
...
ご興味ある方は私のHP（関数型玩具製作所：@<href>{https://www.polymony.net）を見て連絡頂けると幸いです。, https://www.polymony.net）を見て連絡頂けると幸いです。}  
...
次回はこの電子書籍リーダの自作方法についての同人誌（言語はHaskell）を書く予定です @<fn>{5} @<fn>{6}。
...
//footnote[1][自分も速読術についてはよく分かっていません。極めれば専門外の専門書も読めるとの声がありますが懐疑的です。]

//footnote[2][文脈に対応した着色をEmacs（プログラミングエディタ）に倣い「モード」と呼んでいます。Haskellモードとかmarkdownモードとかそういうのです。文法に従い着色とかエラーチェックしてくれます。]

//footnote[3][技術誌的に最後に良いこと言ったと自負していますが、さっそく自己矛盾してる感があります。]

//footnote[4][ペーパー or 電子の選択ではなく、AR技術で紙の文書を着色して読む、というのがひとつの完成形と見ています。]

//footnote[5][先ほどの特許云々はなんだという感じですが、個人が個人で自分で作る分には特許とか問題にならないですし、仮に問題になったとしても権利主張するつもりは全くないです（ここで明言します）。]

//footnote[6][プログラミング教育の題材としても、学習者のモチベーションの点で筋が良いかなと思っています。]
